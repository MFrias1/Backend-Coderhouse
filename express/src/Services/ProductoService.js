...
const getAllWithFilters = async ({ limit = 10, page = 1, query = {}, sort = 'asc' }) => {
    try {
        ...
        return {
            ...
            hasPrevPage,
            hasNextPage,
            prevLink: prevPage ? `/products?limit=${parsedLimit}&page=${prevPage}&sort=${sort}&query=${JSON.stringify(query)}` : null,
            nextLink: nextPage ? `/products?limit=${parsedLimit}&page=${nextPage}&sort=${sort}&query=${JSON.stringify(query)}` : null,
        };
    } catch (error) {
        console.error(error);
        return { status: 'error', message: 'An error occurred while fetching products' };
    }
};
